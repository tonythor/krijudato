...
Tony \## Abstract \## Introduction \## Data Cleaning Tony
...
What trends can we observe regarding language usage over the years?

As shown in the graph below, Python and SQL are the most prevalent languages in 2022. Notably, Julia appears to have exhibited consistent growth since 2017, which makes sense considering it is a relatively new language.

```{r growth-language-time, warning = FALSE, message = FALSE}
...
The plot below provides a clear visualization of the distribution of each language over the years, versus the previous graph which more clearly visualized growth of languages. Notably, in the plot, it is evident that the adoption of Julia has shown steady growth since 2017.

```{r dist-languages-over-years-plot, warning = FALSE, message = FALSE}
